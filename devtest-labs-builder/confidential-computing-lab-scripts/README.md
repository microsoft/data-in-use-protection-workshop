...
This automation script will deploy on Azure one Windows Server 2016 virtual machine (VM), and two Ubuntu Server 18.04 virtual machines (VMs), using for the latter an SSH key to connect.
...
Files in **ARM-templates** are specifications of VMs' types, for the sake of this hands-on lab, one Windows Server 2016 VM and two Ubuntu Server 18.04 VMs as indicated above. These are purely generic templates that can be reused by any project, as deployment-bound values aren't specified here.
